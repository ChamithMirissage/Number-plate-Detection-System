...
def _main():
    ...
    with open(annotation_path_train) as f1:
        lines_train = f1.readlines()
    with open(annotation_path_valid) as f2:
        lines_valid = f2.readlines()
    #np.random.seed(10101)
    #np.random.shuffle(lines)
    #np.random.seed(None)
    num_val = int(len(lines_valid)) #int(len(lines)*val_split)
    num_train = int(len(lines_train)) #len(lines) - num_val
    ...
    if True:
        ...
        model.fit_generator(data_generator_wrapper(lines_train, batch_size, input_shape, anchors, num_classes),
                ...
                validation_data=data_generator_wrapper(lines_valid, batch_size, input_shape, anchors, num_classes),
    ...
    if True:
        ...
        batch_size = 4 # note that more GPU memory is required after unfreezing the body #32
        print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
        model.fit_generator(data_generator_wrapper(lines_train, batch_size, input_shape, anchors, num_classes),
            steps_per_epoch=max(1, num_train//batch_size),
            validation_data=data_generator_wrapper(lines_valid, batch_size, input_shape, anchors, num_classes),
